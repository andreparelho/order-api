...
type OrderRepository interface {
	...
	UpdateOrder(ctx context.Context, order Order, redisKey string) error
}
...
func (o *order) InsertOrder(ctx context.Context, order Order, redisKey string) (bool, error) {
	err := o.redis.Get(ctx, redisKey)
	if err != nil {
		fmt.Printf("\n[WARN]: nao foi possivel buscar o dado %s do cache. Criando dado na base de dados.", redisKey)
	} else {
		fmt.Printf("\n[INFO]: dado encotrado no cache.")
		return true, nil
	}
	...
	if err != nil {
		fmt.Printf("\n[ERROR]: erro ao inserir os dados no banco, erro: %v", err)
		return false, err
	}
	...
	if err != nil {
		fmt.Printf("\n[ERROR]: erro ao realizar marshal, erro: %v", err)
		return false, err
	}

	err = o.redis.Set(ctx, redisKey, orderInfoMarsh, 10*time.Minute)
	if err != nil {
		fmt.Printf("\n[ERROR]: erro ao inserir no cache, erro: %v", err)
		return false, err
	}
...
func (o *order) UpdateOrder(ctx context.Context, order Order, redisKey string) error {
	result, err := o.database.ExecContext(ctx, "UPDATE orders SET status = ? WHERE id = ?", order.Status, order.OrderID)
	if err != nil {
		fmt.Printf("\n[ERROR]: erro ao inserir os dados no banco. Erro: %v", err)
		return err
	}
	...
	if err != nil {
		fmt.Printf("\n[ERROR]: erro ao atualizar o dado no banco de dados. Erro: %v", err)
		return err
	}

	err = o.redis.Del(ctx, redisKey)
	if err != nil {
		fmt.Printf("\n[ERROR]: erro ao deletar o dado do cache. Erro: %v", err)
		return err
	}
